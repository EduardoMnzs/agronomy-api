...
def llm_completion(model, prompt, chat_history=None, return_finish_reason=False):
    ...
    for i in range(max_retries):
        try:
            ...
            if i < max_retries - 1:
                is_rate_limit = "429" in str(e) or "Too Many Requests" in str(e) or "RateLimitError" in str(e)
                wait = min(60 * (2 ** i), 300) if is_rate_limit else 1
                time.sleep(wait)
            else:
...
def extract_json(content):
    # Guard against LLM returning None/empty after max retries exhaust.
    if not content or not isinstance(content, str):
        logging.error("extract_json received empty/None content; returning {}")
        return {}
    try:
